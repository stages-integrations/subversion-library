...
import de.methodpark.subversion.Log;
...
public abstract class AbstractRepositoryCombinedOperationsIT {
    ...
    public void test01_AddFileAndSetProperties() throws Exception {
        ...
        final ResourceProperty property = new ResourceProperty(ResourceProperty.Type.SUBVERSION_CUSTOM, "foo", "bar");
        ...
        try {
            ...
            Assert.assertEquals("change set must contain: " + resource, Transaction.Status.ADDED, transaction.getChangeSet().get(resource));
    ...
    public void test02_SetAndDeleteProperties() throws Exception {
        ...
        final ResourceProperty propertyA = new ResourceProperty(ResourceProperty.Type.SUBVERSION_CUSTOM, "propertyA", "a");
        final ResourceProperty propertyB = new ResourceProperty(ResourceProperty.Type.SUBVERSION_CUSTOM, "propertyB", "b");
        final ResourceProperty propertyC = new ResourceProperty(ResourceProperty.Type.SUBVERSION_CUSTOM, "propertyC", "c");
    ...
    public void test03_ModifyAndDeleteExisting() throws Exception {
        ...
        try {
            repository.add(transaction, resource, true, IOUtils.toInputStream(content, StandardCharsets.UTF_8));
            Assert.assertEquals("change set must contain: " + resource, Transaction.Status.MODIFIED, transaction.getChangeSet().get(resource));
            repository.delete(transaction, resource);
            Assert.assertEquals("change set must contain: " + resource, Transaction.Status.DELETED, transaction.getChangeSet().get(resource));
