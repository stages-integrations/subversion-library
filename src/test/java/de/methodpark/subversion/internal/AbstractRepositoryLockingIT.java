...
public abstract class AbstractRepositoryLockingIT {
    ...
    public void test04_FileCopyToLocked() throws Exception {
        ...
        try {
            ...
            Assert.assertTrue("transaction must be active", transaction.isActive());
            Assert.assertEquals("change set must contain: " + target, Transaction.Status.MODIFIED, transaction.getChangeSet().get(target));
    ...
    public void test04_SetPropertiesOfLocked() throws Exception {
        ...
        final ResourceProperty propertyA = new ResourceProperty(ResourceProperty.Type.SUBVERSION_CUSTOM, "test", "A");
        final ResourceProperty propertyB = new ResourceProperty(ResourceProperty.Type.SUBVERSION_CUSTOM, "test", "B");
