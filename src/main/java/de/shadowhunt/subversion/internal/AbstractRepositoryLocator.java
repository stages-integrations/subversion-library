...
public abstract class AbstractRepositoryLocator implements RepositoryLocator {
    ...
    protected static UUID determineRepositoryId(final URI repository, final Resource prefix, final HttpClient client, final HttpContext context) {
        final QualifiedResource qualifiedResource = new QualifiedResource(Resource.ROOT);
        final InfoOperation operation = new InfoOperation(repository, Resource.ROOT, qualifiedResource, prefix, REPOSITORY_UUID);
        final Optional<Info> info = operation.execute(client, context);
        final Info infoElement = info.orElseThrow(() -> new SubversionException("No repository found at " + repository, HttpStatus.SC_BAD_REQUEST));
        return infoElement.getRepositoryId();
    }
}
